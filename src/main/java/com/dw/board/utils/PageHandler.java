...
public class PageHandler {
	...
	private int pageSize; // 1페이지에 게시물 몇 개 표시할지
	private int startPage; // 현재 블록 첫페이지
	private int endPage; // 현재 블록 마지막 페이지
	private boolean hasNextPage = true; // 이전 버튼 유무
	private boolean hasPreviousPage = true; // 다음 버튼 유무
	private int nowBlock; //현재 블록
	private int lastBlock; // 마지막 블록
	private int navigatePages; // 블록에 페이지 몇 개 표시할지 (ex. 1블록 : 1,2,3,4)
